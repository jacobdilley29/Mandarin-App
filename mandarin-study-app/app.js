...
function nextListenRound() {
  ...
  const pool = VOCAB.filter((v) => v.id !== (currentListenItem && currentListenItem.id));
  currentListenItem = pool[Math.floor(Math.random() * pool.length)];
  speak(currentListenItem.ex.hanzi);

  const distractors = VOCAB.filter((v) => v.id !== currentListenItem.id)
    .sort(() => Math.random() - 0.5)
    .slice(0, 3)
    .map((v) => v.ex.en);
  const options = [...distractors, currentListenItem.ex.en].sort(() => Math.random() - 0.5);
...
function onListenAnswer(btn, opt) {
  const correct = opt === currentListenItem.ex.en;
  document.querySelectorAll(".quiz-option").forEach((b) => {
    b.disabled = true;
    if (b.textContent === currentListenItem.ex.en) b.classList.add("correct");
  });
  if (!correct) btn.classList.add("incorrect");
  document.getElementById("btn-listen-next").style.display = "inline-block";
}

document.getElementById("btn-play-listen").addEventListener("click", () => currentListenItem && speak(currentListenItem.ex.hanzi));
...
function nextSpeakWord() {
  const pool = VOCAB.filter((v) => v.id !== (currentSpeakItem && currentSpeakItem.id));
  ...
  document.getElementById("speak-tone-badge").textContent = tone === 5 ? "Neutral tone" : "Tone " + tone;
...
document.getElementById("btn-record").addEventListener("click", async () => {
  ...
  try {
    ...
    const raw = await recordPitchContour(1600, (partial) => {
      drawContour(normalizeContour(partial), ref);
    });
    const norm = normalizeContour(raw);
    drawContour(norm, ref);
    const voiced = raw.filter((f) => f > 0).length;
    status.textContent = voiced > 5 ? "Compare your line (green) with the target shape (gold)." : "Didn't catch much — try again a bit louder or closer to the mic.";
  } catch (err) {

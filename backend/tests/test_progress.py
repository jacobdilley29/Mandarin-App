...
# ---------------------------------------------------------------------------
def test_html_and_service_worker_are_never_cached():
    """index.html and sw.js must always be revalidated.

    FastAPI sends only ETag/Last-Modified by default, which lets browsers cache
    heuristically. A stale index.html names asset bundles that no longer exist,
    and because the PWA registers with `autoUpdate` the resulting
    update-then-reload cycle repeats indefinitely instead of settling.
    """
    ...
    with TestClient(app) as client:
        for path in ("/", "/index.html", "/sw.js", "/registerSW.js"):
            r = client.get(path)
            if r.status_code != 200:
                continue  # frontend not built in this environment
            assert "no-cache" in r.headers.get("cache-control", ""), path


def test_hashed_assets_are_cacheable_forever():
    """Vite content-hashes /assets, so those URLs never change."""
    from fastapi.testclient import TestClient
...
def test_head_on_the_spa_route_is_allowed():
    """A bare @app.get catch-all answers HEAD / with 405, tripping proxies."""
    from fastapi.testclient import TestClient

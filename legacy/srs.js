...
// --- Pronunciation struggle tracking (Speak tab, word mode) -----------
// Tracks how often the speech recognizer failed to match the target word,
// so Home can surface a short "needs practice" list.

const STRUGGLE_KEY = "mandarin_struggle_v1";

function loadStruggle() {
  try {
...
function saveStruggle(state) {
  localStorage.setItem(STRUGGLE_KEY, JSON.stringify(state));
}

function recordPronunciationAttempt(id, wasMatch) {
  const state = loadStruggle();
  const rec = state[id] || { attempts: 0, misses: 0 };
  rec.attempts += 1;
  if (!wasMatch) rec.misses += 1;
  state[id] = rec;
  saveStruggle(state);
}

// Returns up to `limit` ids with the worst miss ratio (min 2 attempts),
// sorted worst-first.
function getStruggleList(limit = 5) {
  const state = loadStruggle();
  return Object.keys(state)
    .map((id) => ({ id, ...state[id], ratio: state[id].misses / state[id].attempts }))
    .filter((r) => r.attempts >= 2 && r.ratio > 0)
